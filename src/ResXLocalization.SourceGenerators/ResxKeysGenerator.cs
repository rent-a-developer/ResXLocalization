...
public sealed class ResxKeysGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        var resXFiles = context
            .AdditionalTextsProvider.Where(static f => f.Path.EndsWith(".resx", StringComparison.OrdinalIgnoreCase))
            .Select(static (f, ct) => ParseResxFile(f, ct))
            .Where(static m => m is not null)
            .Select(static (m, _) => m!.Value);

        var resXDesignerFiles = context
    ...
    )
    {
        ...
        {
            if (
                string.Equals(resXDesignerFile.FileBaseName, resxFile.FileBaseName, StringComparison.OrdinalIgnoreCase)
                && string.Equals(
    ...
    private static bool IsResourceManagerType(
        string? typeName,
        ImmutableHashSet<string> aliases,
        bool importsSystemResources = false
    )
    {
    ...
    private static ResXDesignerFile? ParseResXDesignerFile(AdditionalText text, CancellationToken ct)
    {
        ...
        var resourceManagerAliases = root.DescendantNodes()
            ...
            .Where(static directive =>
                ...
                && IsResourceManagerType(directive.Name?.ToString(), ImmutableHashSet<string>.Empty)
            )
        ...
        {
            if (
                property.Identifier.ValueText != "ResourceManager"
